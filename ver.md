...
### 변경 내용
* **개선된 필터 기준 및 6대 소스 적용 최신 브리핑 게시글 실시간 갱신(Refresh)**
  * 무관 기사 차단 필터 및 마실/그린대로/온통청년/위비티/웰촌/시골투어 6대 출처 수집 정제 적용 완료
  * DB 브리핑 데이터 수집·갱신 수행 (`data/brief.db`)
...
### 비고
* 최신 수집 데이터 100% 정상 인입 및 정제 렌더링 검증 완료
...
### 변경 내용
* 구글 검색 결과 싹쓸이 수집을 위한 **구글 API 연동 모듈(`google.js`) 신규 개발**
* 사용자 제공 API 키 `.env` 저장 및 구글 공식/비정형 키 대응 예외 핸들링 파이프라인 부착
* RSS + 크롤러 + 네이버 API + 구글 API 4중 수집 채널을 3중 필터로 취합하는 구조 완성

### 수정 파일
* src/lib/search/google.js (신규 생성)
* src/jobs/refresh.js
* .env

### 비고
* API CX 미제공 또는 비정형 키 입력 시 콘솔 안내 및 안전 스킵 처리 검증
...
### 변경 내용
* 세상의 광범위한 웹문서 싹쓸이를 위한 **네이버 검색 오픈 API 연동 기능 개발**
* 네이버 블로그/카페/웹문서에서 "숏폼 공모전", "AI 공모전", "여행지원금 한달살기" 키워드 자동 크롤링 모듈(`naver.js`) 구축
* `.env` 키 유무에 따른 에러 핸들링 및 3중 필터 병합 로직 추가

### 수정 파일
* src/lib/search/naver.js (신규 생성)
* src/jobs/refresh.js

### 비고
* API 키 미발급 시 안전하게 스킵되도록 예외 처리 완료
