...
const TAG_RULES = [
  { tag: 'AI_ShortForm', keywords: ['숏폼', '쇼츠', '릴스', '영상', 'video', 'short-form', 'UCC'] },
  { tag: 'AI_Tech', keywords: ['AI', '인공지능', 'LLM', '생성형', 'OpenAI', 'Gemini', '모델', '도구'] },
  { tag: 'Public_Contest', keywords: ['공모', '공모전', '경진대회', '시상', '홍보', '도전', '챌린지'] },
  { tag: 'Regional_Stay', keywords: ['살아보기', '한달살기', '촌캉스', '체류', '귀농', '청년지원', '관광'] },
  { tag: 'Gov_Agency', keywords: ['지자체', '공공기관', '시청', '도청', '군청', '재단', '진흥원', '부처'] },
  { tag: 'General', keywords: [] },
];

function deriveTags(title) {
  const tags = [];
  for (const rule of TAG_RULES) {
    if (rule.tag !== 'General' && rule.keywords.some((k) => title.toLowerCase().includes(k.toLowerCase()))) {
      tags.push(rule.tag);
    }
  }
...
function computeSelectionScore(item, mode, level) {
  ...
  if (mode === 'decision') {
    addIfTag('AI_ShortForm', 10);
    addIfTag('Public_Contest', 8);
    addIfTag('Regional_Stay', 8);
    addIfTag('AI_Tech', 6);
    addIfTheme('AI 숏폼·영상 공모전', 8);
    addIfTheme('지자체·공공기관 홍보 공모', 6);
  } else {
    addIfTag('AI_ShortForm', 8);
    addIfTag('Public_Contest', 6);
    addIfTag('Regional_Stay', 6);
    addIfTag('AI_Tech', 5);
    addIfTheme('AI 숏폼·영상 공모전', 6);
    addIfTheme('지역 체류·살아보기 지원', 5);
  }

  if (level === '5_10') {
    addIfTag('Regional_Stay', 8);
    addIfTag('Public_Contest', 8);
    addIfTheme('지역 체류·살아보기 지원', 6);
    addIfTheme('지자체·공공기관 홍보 공모', 6);
  } else {
...
function computeBreakdown(title, tags, linkStatus, published_at, refDate) {
  ...
  let impact = 60;
  if (tags.includes('AI_ShortForm')) impact += 15;
  if (tags.includes('Public_Contest')) impact += 12;
  if (tags.includes('Regional_Stay')) impact += 12;
  if (tags.includes('AI_Tech')) impact += 10;
  if (tags.includes('Gov_Agency')) impact += 8;
  impact += Math.min(6, tags.length * 2);

  let relevance = 55 + Math.min(12, tags.length * 3);
  if (tags.includes('AI_ShortForm')) relevance += 10;
  if (tags.includes('Public_Contest')) relevance += 8;
  if (tags.includes('Regional_Stay')) relevance += 8;
  if (tags.includes('AI_Tech')) relevance += 6;
  ...
  if (/(공모|접수|마감|모집|도전|상금|지원사업)/.test(t)) urgency += 8;
  if (/(출시|발표|개최|오픈)/.test(t)) urgency += 4;
